...
export default function SettingsScreen() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: C.bg }} contentContainerStyle={{ padding: 20, paddingTop: 60, paddingBottom: 40 }}>
      ...
      {/* APPEARANCE */}
      <Text style={{ fontSize: 10, fontWeight: '700', letterSpacing: 1.5, color: C.text3, marginBottom: 12 }}>APPEARANCE</Text>
      <View style={{ backgroundColor: C.surface, borderRadius: 14, borderWidth: 1, borderColor: C.border, marginBottom: 24 }}>
      ...
      {/* LANGUAGE */}
      <Text style={{ fontSize: 10, fontWeight: '700', letterSpacing: 1.5, color: C.text3, marginBottom: 8 }}>APP LANGUAGE</Text>
      ...
      {/* API KEYS */}
      <Text style={{ fontSize: 10, fontWeight: '700', letterSpacing: 1.5, color: C.text3, marginBottom: 12 }}>API KEYS</Text>
      {apiKeys.length === 0 && (
        ...
        <View key={k.id} style={{ backgroundColor: C.surface, borderRadius: 12, borderWidth: 1, borderColor: C.border, padding: 14, marginBottom: 10 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ fontSize: 11, color: C.text2, marginTop: 6 }}>{k.key.substring(0, 8)}{'*'.repeat(12)}</Text>
            </View>
            <TouchableOpacity
              onPress={() => Alert.alert('Delete?', 'Remove ' + k.service + ' API key?', [
                { text: 'Cancel' },
      ...
      {showAddKey ? (
        <View style={{ backgroundColor: C.surface, borderRadius: 14, borderWidth: 1, borderColor: C.accent + '44', padding: 16, marginBottom: 12 }}>
          ...
          <TextInput value={newService} onChangeText={setNewService} placeholder="e.g. Claude, Pexels, Pixabay..."
            ...
            style={{ backgroundColor: C.surface2, borderRadius: 8, padding: 10, color: C.text, borderWidth: 1, borderColor: C.border, marginBottom: 12 }} />
          ...
          <TextInput value={newPurpose} onChangeText={setNewPurpose} placeholder="e.g. Script generation, Stock videos..."
            placeholderTextColor={C.text3}
            style={{ backgroundColor: C.surface2, borderRadius: 8, padding: 10, color: C.text, borderWidth: 1, borderColor: C.border, marginBottom: 16 }} />
        ...
        <TouchableOpacity onPress={() => setShowAddKey(true)}
          style={{ backgroundColor: C.surface2, borderRadius: 12, padding: 14, alignItems: 'center', borderWidth: 1.5, borderColor: C.border, borderStyle: 'dashed', marginBottom: 24 }}>
          <Text style={{ color: C.accent, fontWeight: '600' }}>+ Add New API Key</Text>
        </TouchableOpacity>
      )}

      {/* CHANNEL INFO */}
      <Text style={{ fontSize: 10, fontWeight: '700', letterSpacing: 1.5, color: C.text3, marginBottom: 12 }}>CHANNEL INFO</Text>
      <View style={{ backgroundColor: C.surface, borderRadius: 14, borderWidth: 1, borderColor: C.border, padding: 16, marginBottom: 24 }}>
        ...
        <TextInput defaultValue="Kahani Paison Ki"
          style={{ backgroundColor: C.surface2, borderRadius: 8, padding: 10, color: C.text, borderWidth: 1, borderColor: C.border }} />
      </View>

      <TouchableOpacity
        onPress={() => Alert.alert('🗑️', 'Cache cleared')}
