  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: C.bg }} contentContainerStyle={{ paddingBottom: 20 }}>
      ...
      <View style={{ padding: 20, paddingTop: 60 }}>
        <Text style={{ fontWeight: '800', fontSize: 24, color: C.text }}>🔥 Solo Content Forge</Text>
        <Text style={{ fontSize: 13, color: C.text2, marginTop: 4 }}>Kahani Paison Ki — Idea se upload tak</Text>
      </View>

      <View style={{ marginHorizontal: 16, padding: 12, backgroundColor: 'rgba(240,192,64,0.08)', borderRadius: 10, borderWidth: 1, borderColor: 'rgba(240,192,64,0.2)', marginBottom: 20 }}>
        <Text style={{ fontSize: 12, color: '#f0c040' }}>⚙️ API keys not set — add in Settings to enable AI</Text>
      </View>
      ...
      <View style={{ flexDirection: 'row', gap: 10, paddingHorizontal: 16, marginBottom: 20 }}>
        {[['💡', videos.length, 'Ideas'], ['📝', drafted, 'Drafted'], ['🎬', ready, 'Ready']].map(([icon, num, label]) => (
          <View key={label} style={{ flex: 1, backgroundColor: C.surface, borderRadius: 12, padding: 14, alignItems: 'center', borderWidth: 1, borderColor: C.border }}>
            <Text style={{ fontSize: 24, fontWeight: '800', color: C.accent }}>{num}</Text>
      ...
      <View style={{ flexDirection: 'row', gap: 10, paddingHorizontal: 16, marginBottom: 24 }}>
        {[['🎬', videos.length, '#e07840'], ['📱', videos.filter(v => v.type === 'short').length, '#4080e0'], ['🚀', uploaded, '#40c070']].map(([icon, num, color], i) => (
          <View key={i} style={{ flex: 1, backgroundColor: C.surface, borderRadius: 12, padding: 14, alignItems: 'center', borderWidth: 1, borderColor: C.border }}>
            <Text style={{ fontSize: 24, fontWeight: '800', color }}>{num}</Text>
            <Text style={{ fontSize: 10, color: C.text3, marginTop: 4 }}>{icon}</Text>
          </View>
        ))}
      </View>
      ...
      <View style={{ paddingHorizontal: 16, marginBottom: 20 }}>
        <TouchableOpacity
          style={{ backgroundColor: '#f0c040', borderRadius: 14, padding: 16, alignItems: 'center' }}
      ...
      {recent.map(v => (
        <TouchableOpacity key={v.id} onPress={() => navigation.navigate('Library')}
          ...
          <View style={{ width: 40, height: 40, borderRadius: 10, backgroundColor: C.surface2, alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
            <Text style={{ fontSize: 18 }}>{statusIcon[v.status]}</Text>
          </View>
          <View style={{ flex: 1 }}>
