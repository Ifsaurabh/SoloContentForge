...
export default function NewVideoScreen() {
  ...
  const S = { padding: 20, paddingTop: 60, flex: 1, backgroundColor: C.bg };
  const label = { fontSize: 10, fontWeight: '700', letterSpacing: 1.5, color: C.text3, marginBottom: 8 };
  const title = { fontSize: 20, fontWeight: '800', color: C.text, marginBottom: 4 };
  const sub = { fontSize: 13, color: C.text2, marginBottom: 16 };
  ...
  const StepDots = () => (
    <View style={{ flexDirection: 'row', gap: 4, padding: 16, paddingTop: 56 }}>
      {[1,2,3,4,5,6].map(i => (
        <View key={i} style={{ height: 6, width: step === i ? 20 : 6, borderRadius: 100, backgroundColor: i < step ? C.success : i === step ? C.accent : C.border }} />
      ))}
    </View>
  );
  ...
  if (step === 1) return (
    <ScrollView style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      <View style={{ padding: 20, paddingTop: 0 }}>
        <Text style={label}>STEP 1 — YOUR IDEA</Text>
        <Text style={title}>Kya idea hai? 💡</Text>
        <Text style={sub}>Works offline — type and save anytime</Text>
        <View style={{ backgroundColor: C.surface, borderRadius: 12, padding: 16, borderWidth: 1, borderColor: C.border, marginBottom: 16 }}>
          <TextInput
            multiline value={idea} onChangeText={setIdea}
            placeholder="Apna idea likhein..." placeholderTextColor={C.text3}
            style={{ color: C.text, fontSize: 15, minHeight: 80, lineHeight: 22 }}
          />
  ...
  if (step === 2) return (
    <ScrollView style={{ flex: 1, backgroundColor: C.bg }}>
      <StepDots />
      <View style={{ padding: 20, paddingTop: 0 }}>
        <Text style={label}>STEP 2 — VIDEO TYPE</Text>
        <Text style={title}>Kaunsa format? 📱</Text>
        <Text style={sub}>Script structure is different for each</Text>
        <View style={{ flexDirection: 'row', gap: 12, marginBottom: 16 }}>
          {[['short', '📱', 'SHORT', '30 – 90 seconds'], ['long', '🎬', 'LONG', '5 – 15 minutes']].map(([type, icon, lbl, sl]) => (
            <TouchableOpacity key={type} onPress={() => setVideoType(type)}
              style={{ flex: 1, padding: 20, borderRadius: 14, borderWidth: 1.5, borderColor: videoType === type ? C.accent : C.border, backgroundColor: videoType === type ? C.accent + '15' : C.surface2, alignItems: 'center' }}>
        ...
        <TextInput
          ...
          style={{ backgroundColor: C.surface2, borderRadius: 10, padding: 12, color: C.text, borderWidth: 1, borderColor: C.border, marginBottom: 16 }}
        />
  ...
  if (step === 3) return (
    <ScrollView style={{ flex: 1, backgroundColor: C.bg }}>
      <StepDots />
      <View style={{ padding: 20, paddingTop: 0 }}>
        <Text style={label}>STEP 3 — CONTENT PILLAR</Text>
        <Text style={title}>Kaunsa pillar? 🏛️</Text>
        <Text style={sub}>AI style adjusts per pillar</Text>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
          {pillars.map(p => (
            <TouchableOpacity key={p.name} onPress={() => setPillar(p.name)}
              style={{ flexDirection: 'row', alignItems: 'center', gap: 8, padding: 14, borderRadius: 10, borderWidth: 1.5, borderColor: pillar === p.name ? C.accent : C.border, backgroundColor: pillar === p.name ? C.accent + '15' : C.surface2, width: '48%' }}>
  ...
  if (step === 4) return (
    <ScrollView style={{ flex: 1, backgroundColor: C.bg }}>
      <StepDots />
      <View style={{ padding: 20, paddingTop: 0 }}>
        <Text style={label}>STEP 4 — EMOTION SELECTOR</Text>
        <Text style={title}>Feel choose karo 🎭</Text>
        <Text style={sub}>Har section ke liye emotion tap karo</Text>
        {sections.map(sec => (
            ...
            <View style={{ padding: 12, backgroundColor: C.surface, flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {sec.emojis.map(em => (
                <TouchableOpacity key={em.l} onPress={() => setEmotions(prev => ({ ...prev, [sec.id]: em.l }))}
                  style={{ width: '47%', padding: 12, borderRadius: 10, borderWidth: 1.5, borderColor: emotions[sec.id] === em.l ? C.accent : C.border, backgroundColor: emotions[sec.id] === em.l ? C.accent + '15' : C.surface2, alignItems: 'center' }}>
  ...
  if (step === 6) return (
    <ScrollView style={{ flex: 1, backgroundColor: C.bg }}>
      <View style={{ padding: 20, paddingTop: 60, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <TouchableOpacity onPress={() => { addVideo({ title: idea, pillar, type: videoType, status: 'draft' }); Alert.alert('✅', 'Script approved! Video saved to library.'); setStep(1); setIdea(''); setVideoType(''); setPillar(''); setEmotions({}); }}
          style={{ backgroundColor: C.success + '22', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderColor: C.success }}>
          <Text style={{ color: C.success, fontWeight: '600', fontSize: 13 }}>✅ Approve</Text>
        </TouchableOpacity>
      </View>
      <View style={{ backgroundColor: 'rgba(240,192,64,0.08)', marginHorizontal: 16, padding: 10, borderRadius: 8, borderWidth: 1, borderColor: 'rgba(240,192,64,0.2)', marginBottom: 12 }}>
        <Text style={{ fontSize: 12, color: C.accent }}>🤖 Demo mode — add Claude API key in Settings for live AI</Text>
      </View>
      {script.map((sec, idx) => {
        ...
        return (
          <View key={sec.id} style={{ borderWidth: 1, borderColor: C.border, borderRadius: 14, marginHorizontal: 16, marginBottom: 12, overflow: 'hidden' }}>
            ...
            <View style={{ padding: 14, backgroundColor: C.surface }}>
              <TextInput
                multiline value={sec.text}
                onChangeText={val => setScript(prev => prev.map((s, i) => i === idx ? { ...s, text: val } : s))}
                style={{ color: C.text, fontSize: 14, lineHeight: 22, marginBottom: 12 }}
              />
              <View style={{ flexDirection: 'row', gap: 8 }}>
                <TouchableOpacity style={{ flex: 1, padding: 8, borderRadius: 8, backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border, alignItems: 'center' }}
                  onPress={() => { const demo = demoScript; if (demo[idx]) setScript(prev => prev.map((s, i) => i === idx ? { ...s, text: demo[idx].text } : s)); }}>
                  <Text style={{ fontSize: 12, color: C.text2 }}>🔄 Regen</Text>
                </TouchableOpacity>
                <TouchableOpacity style={{ flex: 1, padding: 8, borderRadius: 8, backgroundColor: C.success + '15', borderWidth: 1, borderColor: C.success, alignItems: 'center' }}>
